...
public class FireWeapon : MonoBehaviour
{
    ...
    void Shoot()
    {
        ...
        if (Physics.Raycast(_rayOrigin.position, rayDirection, out _objectHit, _shootDistance, _hitLayers))
        {
            ...
            // Assumes the environment was hit
            else if (_objectHit.transform.tag == "Environment")
            {
    ...
    void NewHitFX(GameObject hitFX)
    {
        ...
        if (_currentHitFX.TryGetComponent<ParticleSystem>(out ParticleSystem particleSystem))
        {
            ...
            /*
            Gradient currentGradient = particleSystem.colorBySpeed.color.gradient;
            Color gradientMax = currentGradient.colorKeys.

            gradient.SetKeys
                (new GradientColorKey[] {
                new GradientColorKey(colorRemap, 0.25f),
                new GradientColorKey(gradientMax, 1.0f) },
                new GradientAlphaKey[] { new GradientAlphaKey(1.0f, 0.0f) });
            */
